...
> "%flashing_report_path%\dfu_output.txt" (
    echo Output file cleared.
)
> "%flashing_report_path%\dfu_success.txt" (
    echo Success file cleared.
)
timeout /t 10 /nobreak >nul
echo Start flashing the DFU file
start "PythonScript" cmd /c "python dfu_flash.py -d am62pxx-toyota -t hsfs -c dfu_emmc_toyota.cfg > "%flashing_report_path%\dfu_output.txt" 2>&1"
timeout /t 900 /nobreak >nul
echo Checking for success messages...
findstr /i "Flashed 1 out of 1 identified devices successfully" "%flashing_report_path%\dfu_output.txt" > "%flashing_report_path%\dfu_success.txt"
if exist "%flashing_report_path%\dfu_success.txt" (
    echo SUCCESS messages have been written to dfu_success.txt
) else (
