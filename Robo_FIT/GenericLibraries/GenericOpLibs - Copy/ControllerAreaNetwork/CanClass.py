...
class CanClass(ICan):
    ...
    def receive_can_message_data1(self, can_des: str, database_num: int = 1, timeout: int = 10, is_list: bool = True,
                                 is_dict: bool = False) -> Union[List, Dict]:
        """
        This function returns data for a CAN message based on the CAN description provided in the CAN_input_Excel.xlsx.

        :param can_des: CAN description from the Excel file.
        :param database_num: Database/channel number (default is 1).
        :param timeout: Timeout duration in seconds (default is 10).
        :param is_list: If True, return the data as a list.
        :param is_dict: If True, return the data as a dictionary.
        :return: The data in the format specified by either is_list or is_dict.

        Note: Only one of is_list or is_dict should be True at a time. If both are set to True,
        this will result in a conflict and the function will prioritize 'is_list' over 'is_dict'.
        """

        # Ensure only one of is_list or is_dict is True
        if is_list and is_dict:
            raise ValueError("Only one of 'is_list' or 'is_dict' can be True.")

        if self.can_instance is None:
            raise TypeError("CAN instance is not initialized. Please check the logs.")

        # Call the corresponding method on the CAN instance
    ...
    def process_hex_to_bin(self, task: can.CyclicSendTaskABC):
        ...
        robot_print_info(f"Value at task[1]: {hex_value}")
        ...
        # Convert the integer to a binary string (8-bit padding)
        bin_value = f"{int_value:08b}"
        # Print the binary value
        robot_print_info(f"Binary Value: {bin_value}")
    ...
    def pulse_0(self, task: can.CyclicSendTaskABC):
        if not isinstance(task, list):
            raise TypeError("The task should be a list of hexadecimal values.")

        if len(task) <=6:
            print("The task does not have an index 11.")
            return False

        hex_value = task[5]
        # Print the value and its type
        robot_print_info(f"Value at task[5]: {hex_value}, Type: {type(hex_value)}")
        ...
        if bin_value[1:3] == '00':
            robot_print_info("pulse 0 check: Pass")
            return True
        else:
            robot_print_error("pulse 0 check: Fail")
            return False
    def pulse_1(self, task: can.CyclicSendTaskABC):
        if not isinstance(task, list):
            raise TypeError("The task should be a list of hexadecimal values.")
        ...
        if bin_value[1:3] == '01':
            ...
            robot_print_info("pulse 1 check: Pass")
            return True
        else:
            robot_print_error("pulse 1 check: Fail")
            return False
    def pulse_2(self, task: can.CyclicSendTaskABC):
        if not isinstance(task, list):
            raise TypeError("The task should be a list of hexadecimal values.")
        ...
        if bin_value[1:3] == '10':
            robot_print_info("pulse 2 check: Pass")
            return True
        else:
            robot_print_error("pulse 2 check: Fail")
            return False
    def pulse_3(self, task: can.CyclicSendTaskABC):
        if not isinstance(task, list):
            raise TypeError("The task should be a list of hexadecimal values.")
        ...
        if bin_value[1:3] == '11':
            robot_print_info("pulse 3 check: Pass")
            return True
        else:
            robot_print_error("pulse 3 check: Fail")
            return False
