...
class ConfiguratorReader:
    ...
    def __init__(self):
        ...
        'Robo_FIT/GenericLibraries/GenericOpLibs/AndroidKeyEvents/adb_key_code_config.json' path.
        """
        try:
            ...
            if os.path.isfile(self.config_path):
                ...
                default_config_path = os.path.join(self.common_keyword.get_root_path(), ROBO_FIT, ADB_KEY_EVENT,
                                                   ADB_KEY_CONFIG)
                self.config_list = self.read_configuration(default_config_path)
            # self.config_list = ""
            # if config_path.strip() != "":
            #     # path = get_suite_path() + config_path
            #     path = os.path.join(self.common_keyword.get_suite_path(), config_path)
            #     self.config_list = self.read_configuration(path)
            # else:
            #     print("Please provide Global Configuration path for ADB logger")
            #     logger.console("Please provide Global Configuration path for ADB logger")
            #     # default_config_path = get_suite_path() +
            #     # "Framework_Components/ADBLogger/adblogger_configuration_file.json"
            #     default_config_path = os.path.join(self.common_keyword.get_suite_path(), "Robo_FIT", "ADBLogger", "adblogger_configuration_file.json")
            #     self.config_list = self.read_configuration(default_config_path)
