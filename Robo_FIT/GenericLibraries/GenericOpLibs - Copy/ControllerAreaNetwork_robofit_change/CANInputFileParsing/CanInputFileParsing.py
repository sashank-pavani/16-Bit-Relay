...
class CanInputFileParsing:
        ...
        try:
            ...
            if channel_name == "single":
                ...
                try:
                    ...
                    # robot_print_info(f"self.database_single ----******{self.database_single}")
                except Exception as e:
                    robot_print_error(f"error found in self.database_single  - {e}")
    ...
    def get_can_message_name(self, can_des: str, database_num: int, signal_type: str = "TX") -> str:
        ...
        """
        try:
            robot_print_info(f"Reached here flow 3*** ")
            ...
            robot_print_info(f"Reached here flow 4*** {user_given_can_message_name.isnull().values.any()}")
            if user_given_can_message_name.isnull().values.any():
                ...
                robot_print_info(f"Reached here flow 5 if*** ")
                ...
                if self.__is_can_message_name_in_dbc(user_given_can_message_name.item(),
                    ...
                    robot_print_info(f"Reached here flow 5 elif*** ")
                    ...
                    robot_print_info(f"Reached here flow 5 else*** ")
    ...
    def __is_can_message_name_in_dbc(self, user_given_can_message_name: str, database_num: int) -> bool:
        ...
        """
        try:
            robot_print_info(f"Reached here flow 6 if*** ")
            if database_num == 1:
    ...
    def __is_can_frame_signal_choices_valid(self, msg: cantools.db.Message, database_num, json_data: dict,
        ...
        try:
            available_choices_dict = self.get_available_choices(msg)
            robot_print_debug(f" available_choices_dict{available_choices_dict}")
            # check json_data
            for key, value in json_data.items():
                available_choices = available_choices_dict[key]
                robot_print_debug(f" key{key}---value{value}")
                robot_print_debug(f" available_choices{available_choices}")
                if isinstance(available_choices, dict):
                    robot_print_debug(f"available_choices.values(){available_choices.values()}")
        ...
        try:
            ...
            if len(can_data) == msg.length:
                ...
                if self.__is_can_frame_signal_choices_valid(msg, database_num, json_data, can_des, signal_type):
                    robot_print_info(f"reached line number393....")
                    return bytearray(can_data)
                else:
                    robot_print_info(f"reached line number396....else")
                    raise Exception(
...
class CanDatabase:
    ...
    def get_can_db_instance(db_file_path) -> cantools.database.Database:
        ...
        """
        robot_print_info("f Reached flow 6")
    ...
    def __init__(self, db_file_path):
        if db_file_path not in CanDatabase._instances:
            try:
                robot_print_info("f Reached flow 7")
                try:
                    robot_print_info("f Reached flow ******ERROR**************")
                    self.can_db = cantools.database.load_file(db_file_path)
                    # robot_print_info(f"self.can_db {self.can_db}")
                    # robot_print_info(f"self.can_db {self.can_db}*****************************************************************************************************")
                    CanDatabase._instances[db_file_path] = self
                    # robot_print_info(f"self.can_db ******************** This is NO{self.can_db}NE")
                except cantools.Error as can_tools_error:
                    robot_print_info(f"Reached flow  ************7 try with Error No-Ne")
                    print("There is an error while reading the CAN Database files:\nException %s" % can_tools_error)
            except cantools.Error as can_tools_error:
                robot_print_info("f Reached flow  except 7")
                print("There is an error while reading the CAN Database files:\nException %s" % can_tools_error)
        else:
