...
class ConvertXmlToJson:
    ...
    def __read_xml_file(self):
        ...
        try:
            ...
            robot_print_error(f"Error to read the xml file = {self.xml_file_path}, EXCEPTION: {exp}")
    ...
    def __get_jira_key_from_test_case(self, test_case) -> str:
        """
        This function is used to get jira keys from the user Test case Title
        :param test_case: Test case Title
        :type test_case: String
        :return: Test case Jira Key
        :rtype: String
        """
        try:
            """
            The First Priority is to the test case Name in .robot
            
            If test case name is of "<jira_test_key> <jira_test_summary>" format, 
            then first priority to take the test key is '<jira_test_key'>
            
            If test case tag is of "TEST_<jira_test_key>' format,
            then test key become '<jira_test_key>' takes from Test case Tag (this changes for BDD)
            """
            test = test_case.get("name").split(" ")
            if len(test) >= 2:
                # validating the JIRA test case id
                if len(self.jira_api_handler.get_issue_by_id_key(test[0])) != 0:
                    return test[0]
                else:
                    robot_print_error(f"The key: \"{test[0]}\" is not a valid JIRA TEST CASE ID")
            else:
                for tag in test_case.find_all("tag"):
                    if "TEST_" in tag.text:
                        return tag.text.split("_")[-1]
            robot_print_error(
                f"It seems in test case: \"{test_case}\" test case ID is not added, so skipping this test case")
        except Exception as exp:
            robot_print_error(f"Error to get the test case ID from the test: \"{test_case}\", EXCEPTION: {exp}")
    ...
    def convert_xml_to_json_new(self) -> Tuple[str, List]:
        """
        ...
        try:
            ...
            for test_case in xml_test_cases:
                test_jira_key = self.__get_jira_key_from_test_case(test_case)
            ...
            return json_obj, self.test_id_list
        except Exception as exp:
