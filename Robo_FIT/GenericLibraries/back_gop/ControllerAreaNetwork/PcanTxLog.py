...
Created this file to enable the Tx messages in the can trace for PCAN device,
Class inherits behaviour of PcanBus inside pcan.py file and overrides its properties(made changes for is_rx parameter)
"""
import logging
import platform
import time
...
class PcanTxLog(PcanBus):
    def __init__(
            ...
            receive_own_messages=False,  # added
            bitrate: int = 500000,
            **kwargs: Any,
    ):
        ...
        super(PcanBus, self).__init__(channel=channel, state=state, bitrate=bitrate, **kwargs)
        if receive_own_messages:
            ...
            if result != PCAN_ERROR_OK:
                if platform.system() != "Darwin":
                    ...
                    #  https://github.com/mac-can/PCBUSB-Library/
                    log.debug(
                        "Ignoring error. PCAN_ALLOW_ECHO_FRAMES is still unsupported by OSX Library PCANUSB v0.11.2"
                    )
    ...
    def _recv_internal(
        ...
        is_rx = not ((
                             pcan_msg.MSGTYPE & PCAN_MESSAGE_ECHO.value
