...
class UnifiedDiagnosticServices:
    ...
    def unlock_security_access1(self, algo_dict: dict, depth: int) -> dict:
        """
    ...
    """
    Changes only in unlock_security_access method , please replace by below method in UnifiedDiagnosticServices.py file
    """

    def unlock_security_access(self, algo_dict: dict, depth: int) -> dict:
        """
        This Function is used for Security Unlock
        ...
        """
        try:
            robot_print_info(f"depth here is >>>>>>:{depth}")
            for key, value in algo_dict.items():
                robot_print_info(f"key :{key}")
                if key == depth:
                    robot_print_info(f"{'-' * 20}SECURITY LEVEL {hex(key)} {'-' * 20}", True)
                    if depth in SECURITY_LEVELS:
                        self.config['security_algo'] = value['algo']
