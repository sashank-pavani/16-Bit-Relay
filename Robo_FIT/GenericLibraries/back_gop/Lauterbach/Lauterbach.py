...
import win32con
import ctypes
# from Robo_FIT.GenericLibraries.GenericOpLibs.VAutoKit.Vautokit.VAutoKitClass import *
# from Robo_FIT.GenericLibraries.GenericOpLibs.VAutoKit.ConfigurationManager import *
# from Robo_FIT.GenericLibraries.GenericOpLibs.VAutoKit.Vautokit.ConfigurationReader import *
...
class Lauterbach:
    ...
    def unzip_dfu_package(self):
        """Load dfu nor file into through command prompt and flash."""

        dfufolder1 = r"C:\Toyota_daily\USB_PKG\Exe\DFU_PACKAGE.tar.gz"
        # destination_folder1 = r"C:\Toyota_release\USB_PKG\Exe\DFU_PACKAGE.tar"
        # destination_folder2 = r"C:\Toyota_release\USB_PKG\Exe\DFU_PACKAGE"
        destination_folder = os.path.dirname(dfufolder1)

        try:
            with tarfile.open(dfufolder1, 'r:gz') as tar_ref:
                # Extract all the contents to the same directory
                tar_ref.extractall(destination_folder)
            return True
        except Exception as exp:
            robot_print_info(f"unzip is not successfully: {exp}")
            return False

    def unzip_UART_package(self):
        """Load dfu nor file into through command prompt and flash."""

        UART_flashfolder1 = r"C:\Toyota_daily\USB_PKG\Exe\UART_PACKAGE.tar.gz"
        # destination_folder1 = r"C:\Toyota_release\USB_PKG\Exe\UART_PACKAGE.tar"
        # destination_folder2 = r"C:\Toyota_release\USB_PKG\Exe\UART_PACKAGE"
        destination_folder = os.path.dirname(UART_flashfolder1)

        try:
            with tarfile.open(UART_flashfolder1, 'r:gz') as tar_ref:
                # Extract all the contents to the same directory
                tar_ref.extractall(destination_folder)
            return True
        except Exception as exp:
    ...
    def flash_uart_nor_file(self):
        """Load dfu nor file into through command prompt and flash."""
        windows = gw.getWindowsWithTitle("cmd.exe")
        if windows:
            windows[0].minimize()
        time.sleep(2)
        batch_script_path = r'C:\TOYOTA_DAILY_SANITY\Robo_FIT\flashnorfile.bat'  # Replace with the actual path to your .bat file
        try:
            os.system(f'start {batch_script_path}')
            return True
        except Exception as exp:
            robot_print_info(f"Nor flashing command not sent properly: {exp}")
            return False

    def flash_dfu_emmc_file(self):
        """Load dfu nor file into through command prompt and flash."""
        windows = gw.getWindowsWithTitle("cmd.exe")
        if windows:
            windows[0].minimize()
        ...
        batch_script_path = r'C:\TOYOTA_DAILY_SANITY\Robo_FIT\flashemmcfile.bat'
        try:

            os.system(f'start {batch_script_path}')
            return True
        except Exception as exp:
            robot_print_info(f"EMMC flashing command not sent properly: {exp}")
            return False
    def close_command(self, title: str):

        hwnd = win32gui.FindWindow(None, title)
        if hwnd:
            # Send a close message to the window
            # win32gui.SetForegroundWindow(hwnd)
            time.sleep(3)  # Short delay to ensure the window is in focus
            ...
            print(f"Closed window with title:'{hwnd}'")
            return True
        else:
