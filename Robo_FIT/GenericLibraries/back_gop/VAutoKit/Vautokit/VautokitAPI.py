    ...
    while True:
        try:
            ...
            if len(data) > 1 and data[0] == CMD_RELAY_CTL:
                ...
                rx_idx = data[2] + 5
                i2c_read_data = bytearray()
                #robot_print_info(f"I2CWR1 - {data[i]}")
                for i in range(rx_idx, rx_idx + rx_len):
                    #robot_print_info(f"I2CWR - {data[i]}")
                    i2c_read_data.append(data[i])
                ...
                if can_rx_callback is not None:
                    id = data[1] << 8 | data[2]
                    logger.info(
                        "CMD_CAN_RX : " + format(id, '#06x') + " " + format(data[3], '#04x') + " " + format(data[4],
                                                                                                            '#04x') + " " + format(
                            data[5], '#04x') + " " + format(data[6], '#04x') + " " + format(data[7],
                                                                                            '#04x') + " " + format(
                            data[8], '#04x') + " " + format(data[9], '#04x') + " " + format(data[10], '#04x'))
            ...
            robot_print_info(f"Exception found as {e}")


def init(port, log_file):
    robot_print_info("Connecting to " + port)
    try:
        ser_con.port = port
        ser_con.baudrate = 1000000
        ser_con.bytesize = 8
...
def set_relay(idx, on):
    ...
    resp_event.clear()
    ser_con.write(cmd)
    #robot_print_info(f"serail relay commna :  {cmd}")
    time.sleep(2)
...
def can_rx_mon_enable(enable, callback=None):
    ...
    #     logger.info("CMD_CAN_RX_MON : " + str(enable) + " - Pass")
    #
    #
    # else:
    #     logger.info("CMD_CAN_RX_MON : " + str(enable) + " - Fail")
...
def can_tx(id, data):
    ...
    if response[CMD_CAN_TX] == STS_OK:
        logger.info("CMD_CAN_TX : " + format(id, '#06x') + " " + format(data[0], '#04x') + " " + format(data[1],
                                                                                                        '#04x') + " " + format(
            data[2], '#04x') + " " + format(data[3], '#04x') + " " + format(data[4], '#04x') + " " + format(data[5],
                                                                                                            '#04x') + " " + format(
            data[6], '#04x') + " " + format(data[7], '#04x'))
    else:
        logger.info("CMD_CAN_TX : " + format(id, '#06x') + " " + format(data[0], '#04x') + " " + format(data[1],
                                                                                                        '#04x') + " " + format(
            data[2], '#04x') + " " + format(data[3], '#04x') + " " + format(data[4], '#04x') + " " + format(data[5],
...
def i2c_master_write_read(addr, tx_data, rx_data):
    ...
    #print (len(rx_data),len(i2c_read_data))

    # for i in rx_data:
    #     robot_print_info(f"data1 : ***************{ i}")
    #
    # for i in i2c_read_data:
    #     robot_print_info(f"data2 : ***************{ i}")
