...
class SerialLogging:
    ...
    def __store_serial_log_file_name(self, device: str, file_name: str):
        try:
            if SerialLogging.__serial_files_dict.get(device) is None:
                SerialLogging.__serial_files_dict[device] = []
            SerialLogging.__serial_files_dict[device].append(file_name)
        except KeyError as exp:
            robot_print_error(f"Error to store file name in SerialLogging file {exp}")
                ...
                if SerialLogging.__logfile_one is None or SerialLogging.__logfile_one != file_name:
                    if file_name is None:
                        ...
                        robot_print_debug(
                            ...
                            print_in_report = True)
                self.__store_serial_log_file_name(device, SerialLogging.__logfile_one)
                ...
                if SerialLogging.__logfile_two is None or SerialLogging.__logfile_two != file_name:
                    if file_name is None:
                        ...
                        robot_print_debug(
                            f"Start getting serial logs for device : {device}, in file name: {SerialLogging.__logfile_two}",
                            print_in_report = True)
                self.__store_serial_log_file_name(device, SerialLogging.__logfile_two)
                ...
                if SerialLogging.__logfile_three is None or SerialLogging.__logfile_three != file_name:
                    if file_name is None:
                        ...
                        robot_print_debug(
                            f"Start getting serial logs for device : {device}, in file name: {SerialLogging.__logfile_three}",
                            print_in_report = True)
                self.__store_serial_log_file_name(device, SerialLogging.__logfile_three)
                ...
                if SerialLogging.__logfile_four is None or SerialLogging.__logfile_four != file_name:
                    if file_name is None:
                        ...
                        robot_print_debug(
                            f"Start getting serial logs for device : {device}, in file name: {SerialLogging.__logfile_four}",
                            print_in_report = True)
                self.__store_serial_log_file_name(device, SerialLogging.__logfile_four)
    ...
    def find_string_in_serial_file(self, device: str, expected_string: str) -> Tuple[bool, Tuple[str, int | None]]:
        """
        find the string in serial file
        """
        if SerialLogging.__serial_files_dict.get(device) is None:
            raise ValueError(f"There is no serial file available for given device '{device}', "
                             f"Please use start_getting_serial_log() method to start the serial logs")
        logfile_names = SerialLogging.__serial_files_dict[device]
        for logfile_name in logfile_names:
            ...
            with open(log_file_path, "r") as serial_log_file:
                try:
                    if serial_log_file is not None:
                        for line_number, line in enumerate(serial_log_file, start = 1):
                            if expected_string in line:
                                return line
                    else:
                        robot_print_error(f"Serial file is None for device : {device}, in file path: {log_file_path}")
                        return False, (expected_string, None)
                except FileNotFoundError as exp:
                    robot_print_error(
                        f"Serial file not found for device : {device}, in file path: {log_file_path} Exp : {exp}")
                    return False, (expected_string, None)
                except Exception as nexp:
                    robot_print_error(
                        f"Serial file not found for device : {device}, in file path: {log_file_path} Exp : {nexp}")
                    return False, (expected_string, None)
                finally:
