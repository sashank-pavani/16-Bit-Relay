...
class ExecutionClass:
    ...
    def execute_robot(self, tags_list=None, check_report=False, test_execution_key=None, test_plan_key=None,
                      ...
                      is_convert_manual_to_cucumber=True):

        try:
            report_path = self.__create_report_path()
            final_tag_list = []
            ...
            if is_upload_execution_record:
                ...
                if is_convert_manual_to_cucumber:
                    robot_print_debug(f"User has started with the uploading of test cases with execution record",
                                      print_in_report=True, underline=True)
                    self.upload_test_case_record(test_sub_group=test_sub_group, test_group=test_group,
                                                 test_type_name=test_type_name)
                    xray_upload.start_upload_test_cases_record(test_execution_key=test_execution_key,
                                                               test_plan_key=test_plan_key,
                                                               test_sub_group=test_sub_group, test_group=test_group,
                                                               test_type=test_type_name,
                                                               build_version=self.get_build_info())
                else:
                    robot_print_info(f"User has started the execution without uploading the test cases",
                                     print_in_report=True, underline=True)
            ...
            robot_print_error("Execution interrupted by user. Collecting test artifacts before exit...",
    ...
    def upload_only_execution_record(self, test_execution_key=None, test_plan_key=None, test_sub_group=None,
                                     test_group=None, test_type_name=None, is_upload_execution_record=True,
                                     test_report_path=None, is_convert_manual_to_cucumber=True):
        if is_upload_execution_record:
            report_path = self.__create_report_path()
            from Robo_FIT.GenericLibraries.GenericOpLibs.XrayUtility.XrayExecutionRecordUpload import \
                XrayExecutionRecordUpload
    ...
    def upload_test_case_record(self, test_sub_group=None, test_group=None, test_type_name=None):
        from Robo_FIT.GenericLibraries.GenericOpLibs.XrayUtility.TcUpload.XrayTestCaseUpload import XrayTestCaseUpload
        robot_print_info(f"User has started the upload of the test cases", print_in_report=True, underline=True)
        xray_upload = XrayTestCaseUpload()
        xray_upload.start_upload_test_case(test_sub_group=test_sub_group, test_group=test_group,
                                           test_type=test_type_name)
    ...
    def find_performance_excel_file(self):
        try:
            ...
            if os.path.exists(self.boot_kpi_excel_file):
                ...
                robot_print_info(
                    f"No excel file found. Please check documentation for generating performance log excel",
                    print_in_report=True, underline=True)
                return None
    ...
    def commands_execution(self, report_path, team, tag_list):
        ...
        listener_cls_path = self.__get_listener_cls_path()
        robot_print_debug(
            "{robot_cmd} -b {logfile} {tag_string} -l {path}/test_log.html -r {path}/test_report.html -o {path}/test_output.xml --listener {listener}"
            " {testcase}".format(robot_cmd=robot_cmd, logfile=os.path.join(report_path, "RobotLogs.log"),
                                 testcase=testcases_path,
                                 tag_string=tag_string, path=test_output_dir, listener=listener_cls_path))
        os.system(
            "{robot_cmd} -b {logfile} {tag_string} -l {path}/test_log.html -r {path}/test_report.html -o {path}/test_output.xml --listener {listener}"
            " {testcase}".format(robot_cmd=robot_cmd, logfile=os.path.join(report_path, "RobotLogs.log"),
                                 testcase=testcases_path,
    ...
    def pre_conditions(self):
        if os.path.isfile(os.path.join(self.helper.get_root_path(), ROBO_FIT, ROBO_UDS_ON_CAN_LOG_FILE)):
            os.remove(os.path.join(self.helper.get_root_path(), ROBO_FIT, ROBO_UDS_ON_CAN_LOG_FILE))
    ...
    def collect_TestArtifacts(self):
        ...
        try:
            ...
            if os.path.isfile(os.path.join(self.helper.get_root_path(), ROBO_FIT, ROBO_UDS_ON_CAN_LOG_FILE)):
                shutil.copy(os.path.join(self.helper.get_root_path(), ROBO_FIT, ROBO_UDS_ON_CAN_LOG_FILE),
                            self.helper.get_can_trace_dir())
                robot_print_debug(
                    f"Move Done of file : {os.path.join(self.helper.get_root_path(), ROBO_FIT, ROBO_UDS_ON_CAN_LOG_FILE)} "
            ...
            self.helper.copy_the_vr_excel_in_test_report()
    ...
    def __get_listener_cls_path(self):
        file_path = os.path.join(self.helper.get_root_path(), ROBO_FIT, ROBO_LIBS, ROBO_COMPONENTS, TEST_ARTIFACTS,
                                 TEST_ARTIFACTS_LISTENER, TEST_ARTIFACTS_LISTENER_CLS_NAME)
...
if __name__ == '__main__':
    ...
    def main():
        ...
        parser.add_argument(
            "-mc",
            "--convert-manual-to-cucumber",
            dest="isConvertManualToCucumber",
            help="pass values as 'Yes' if user wants to upload test cases",
            required=True,
            default="Yes"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help=f"Enable the verbose debug logs",
            required=False
        )
        result = parser.parse_args()
        print(result)

        def set_verbose_logs():
            try:
                if result.verbose:
                    os.environ['ROBOFIT_DEBUG_ENABLE'] = "Yes"
                else:
                    is_enable = os.getenv('ROBOFIT_DEBUG_ENABLE')
                    if is_enable is not None:
                        if is_enable.lower() == "yes":
                            os.environ['ROBOFIT_DEBUG_ENABLE'] = "Yes"
                        else:
                            os.environ['ROBOFIT_DEBUG_ENABLE'] = "No"
                    else:
                        os.environ['ROBOFIT_DEBUG_ENABLE'] = "No"
            except Exception:
                os.environ['ROBOFIT_DEBUG_ENABLE'] = "No"
        ...
        def run_execution_with_upload_records(execution_class: ExecutionClass, tags_list,
            ...
            execution_class.execute_robot(tags_list=tags_list, check_report=check_user_choice(),
                                          ...
                                          is_convert_manual_to_cucumber=is_convert_manual_to_cucumber)
        ...
        def upload_record_without_execution(execution_class, report_path, is_convert_manual_to_cucumber):
        ...
        def get_root_path(path):
            if "robofit" in os.listdir(path):
                return os.path.join(path, "robofit")
            elif "Robo_FIT" in os.listdir(path):
                return Path(path)
            else:
                with os.scandir(path) as entries:
                    for entry in entries:
                        if entry.is_dir(follow_symlinks=False) and ".repo" not in entry.name:
                            return entry.path

        print(f"{os.environ['ROBOFIT_DEBUG_ENABLE'] = }")
        try:
            is_valid_team_name = check_is_valid_team_name()
            execution_path = get_root_path(result.path)
            robot_print_debug(f"Execution Path: {execution_path}")
            set_verbose_logs()
            if int(result.execution_option) == 1:
                ...
                if is_valid_team_name:
                    if result.tags is not None:
                        ...
                        robot_print_info(f"{result.isConvertManualToCucumber.upper=}")
                        if result.isUploadExecutionRecord.upper() == "Yes".upper():
                            if result.isConvertManualToCucumber.upper() == "Yes".upper():
                                ...
                                run_execution_with_upload_records(execution_class=execution,
                                                                  tags_list=result.tags.split(","),
                                                                  is_convert_manual_to_cucumber=False)
                ...
                if is_valid_team_name:
                    if not folder_path_exists(result.report_folder):
                        if result.team == "2":
                            report_path = os.path.join(execution_path, PROJECT, TEAM_SWE5, TEST_REPORTS,
                                                       result.report_folder)
                        elif result.team == "6":
                            report_path = os.path.join(execution_path, PROJECT, TEAM_ROBOT_SCRIPT, TEST_REPORTS,
                                                       result.report_folder)
                        else:
                            raise ValueError("Unexpected team name encountered.")
                    else:
