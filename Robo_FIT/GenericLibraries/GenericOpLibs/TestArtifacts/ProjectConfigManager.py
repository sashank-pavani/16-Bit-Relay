...
class ProjectConfigManager:
    ...
    def is_convert_manual_to_cucumber(self) -> bool:
        """
        Determine if manual tests should be converted to Cucumber tests based on the configuration.
        Reads the 'isConvertManualToCucumber' value from the 'xrayImporter' section in the project configuration file.
        :return: The value of 'isConvertManualToCucumber' from the configuration.
        :rtype: bool
        """
        try:
            return bool(self.project_config_file.read_list("xrayImporter")["isConvertManualToCucumber"])
        except Exception as exp:
            robot_print_error(f"Error to read the xrayImporter-> isConvertManualToCucumber key, EXCEPTION: {exp}")
