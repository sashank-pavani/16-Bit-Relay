...
class CommonKeywordsClass:
    ...
    def get_log_screenshot_directory(self, test_name: str) -> str:
        ...
        """
        path = os.path.join(self.get_logs_screenshot_path(), test_name)
        try:
            if not os.path.isdir(path):
    ...
    def get_can_xl_path(self):
        path = os.path.join(self.get_input_files_dir_path(), ROBO_CAN_INPUT_FILE_NAME)
        if not os.path.isfile(path):
            raise FileNotFoundError(f"'{ROBO_CAN_INPUT_FILE_NAME} is not found at '{self.get_input_files_dir_path()}'")
        return path

    def get_can_dbc_file_path(self):
        path = os.path.join(self.get_dbc_dir_path(), ROBO_CAN_INPUT_FILE_NAME)
        if not os.path.isfile(path):
            raise FileNotFoundError(f"'{ROBO_CAN_INPUT_FILE_NAME} is not found at '{self.get_input_files_dir_path()}'")
        return path

    def get_core_services_dir_path(self):
        if self.team_name == TEAM_ROBOT_SCRIPT:
            dir_path = os.path.join(self.get_root_path(), PROJECT, CRE_CORE_SERVICES)
            if os.path.isdir(dir_path):
                return dir_path
            raise NotADirectoryError(f"Given path {dir_path} is not a valid path of directory")
        robot_print_error(f"User is not using the BDD structure so there is no Core Services in old project")
        return None


    def get_appium_logs_file_path(self, port: Union[int, str]):
        try:
            path = os.path.join(self.get_report_path(), "AppiumLogs")
            if not os.path.isdir(path):
                os.makedirs(path)
            return os.path.join(path, f"Appium_Logs_Port_{port}.log")
        except OSError as exp:
            robot_print_error(f"Error to get the appium log file path, EXCEPTION: {exp}")
            raise OSError(f"Error to get the appium log file path, EXCEPTION: {exp}")

    def get_performance_utilization_path(self):
        try:
            path = os.path.join(self.get_report_path(), PERFORMANCE_UTILISATION_DIR)
            if os.path.isfile(path):
                return path
            return None
        except Exception as exp:
    ...
    def get_xray_utility_logs_dir_path(self):
        try:
            path = os.path.join(self.get_report_path(), ROBOFIT_XRAY_UTILITY_LOG_DIR)
            if not os.path.isdir(path):
                os.makedirs(path)
            return path
        except OSError as exp:
            robot_print_error(f"Error to get the Xray Utility logs dir path, EXCEPTION: {exp}")

    def copy_the_vr_excel_in_test_report(self):
        """
        The method will copy the Excel in the test report like
        <CRE/SWE5_SWIntegrationTest/Test_Reports/VoiceRecognition/Reports/voice_recognition_commands.xlsx>
        :param input_file_name: voice_recognition_commands.xlsx
        """
        try:
            input_file = os.path.join(self.get_root_path(), PROJECT, CRE_EXTERNAL_FILES,
                                      CRE_INPUT_FILES, VOICE_RECOGNITION, VOICE_RECOGNITION_EXCEL_NAME)
            if os.path.isfile(input_file):
                CommonKeywordsClass.TEST_MODULE_FOLDER = os.path.join(self.get_report_path(),
                                                                      VOICE_RECOGNITION, ROBOT_REPORT_FOLDER_NAME)
                if not os.path.isdir(CommonKeywordsClass.TEST_MODULE_FOLDER):
                    os.makedirs(CommonKeywordsClass.TEST_MODULE_FOLDER, mode=0o777, exist_ok=True)
                    robot_print_info(f"Created Module directory: {CommonKeywordsClass.TEST_MODULE_FOLDER}")
                destination_file = os.path.join(CommonKeywordsClass.TEST_MODULE_FOLDER,
                                                "voice_recognition_commands.xlsx")
                robot_print_debug(f"Destination file path: {destination_file}")
                if os.path.isfile(destination_file):
                    try:
                        os.remove(destination_file)
                    except IOError as exp:
                        robot_print_error(f"Error while deleting file: {exp}")
                try:
                    ...
                    robot_print_info(f"File copied successfully to {destination_file}")
                except IOError as exp:
                    robot_print_error(f"Error while copying file: {exp}")
            else:
                robot_print_debug(f"Input file does not exist: {input_file}")
        except FileNotFoundError as file_exp:
            robot_print_error(f"Error to find the file, EXCEPTION: {file_exp}")
        except PermissionError as perm_exp:
            robot_print_error(f"PermissionError: {perm_exp}")
        except Exception as exp:
            robot_print_error("EXCEPTION: %s" % exp)
            exc_type, exc_obj, exc_tb = sys.exc_info()
