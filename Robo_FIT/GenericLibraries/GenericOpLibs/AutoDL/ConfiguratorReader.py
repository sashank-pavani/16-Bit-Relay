...
class ConfiguratorReader:
    ...
    def __init__(self):
        ...
        'Robo_FIT/GenericLibraries/GenericOpLibs/ControllerAreaNetwork/robust_config_file.json' path.
        """
        try:
            self.common_keyword = CommonKeywordsClass()
            self.config_path = self.common_keyword.get_path(AUTOFLASH_CONFIG, parent=PROJECT_CONFIG, is_file=True,
                                                            is_dir=False)
    ...
    def read_float(self, key: str) -> float:
        """
        return float value for respective key
        :param key: key to be searched in dictionary
        :return: corresponding value for the key
        """
        try:
