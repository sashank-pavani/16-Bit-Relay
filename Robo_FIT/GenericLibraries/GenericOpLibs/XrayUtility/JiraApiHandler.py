...
from Robo_FIT.GenericLibraries.GenericOpLibs.TestArtifacts.Constants import TEST_CASE_TYPES, TEST_GROUPS, \
    TEST_SUB_GROUPS, ROBOFIT_TC_TRANSITION_STATE
...
class IssueTypeTestData:
    ...
    def __str__(self):
        return f"{self.key} + {self.id} + {self.summary}"
...
class JiraApiHandler:
    ...
    __test_case_status_automation_debug = "Automation Debug"
    __test_case_status_new = "New"
    __test_case_status_released = "Released"
    __test_case_status_implementation_review = "Implementation Review"
    __test_case_status_draft = "Draft"
    __test_case_status_design = "Design"
    __test_case_status_design_review = "Design Review"
    __test_case_status_implementation = "Implementation"
    ...
    # Issue Creation
    ISSUE_CREATION_TE_REQUIRED_KNOWN_FIELDS = ["Issue Type", "Project", "Reporter", "Summary"]
    #ISSUE_CREATION_TE_REQUIRED_KNOWN_FIELDS = ["Issue Type", "Project", "Summary"]
    ...
    def __init__(self):
        ...
        self.__test_data_dict: Dict[str, IssueTypeTestData] = {}
    ...
    def __send_request(self, url, method: str, headers: Dict, payload: Union[Dict, str, None] = None,
                       params: Union[Dict, None] = None) -> Response:
        try:
            if self.auth is None:
                self.auth = self.__create_auth()
            if method == JiraApiHandler.__req_method_get:
            ...
            elif method == JiraApiHandler.__req_method_post or method == JiraApiHandler.__req_method_put:
                if payload is None:
                    raise ValueError(f"Payload can not be None when sending '{method}' request")
                return requests.request(
                    method,
                    url,
                    data=payload,
                ...
                raise ValueError(f"Given method is not supported, {method = }, {url = }")
    ...
    def get_issue_by_summary(self, summary, issueType=ISSUE_TYPE_TEST):
        ...
        try:
            ...
            response = self.__send_request(url=url, method=JiraApiHandler.__req_method_get,
    ...
    def get_state_transition_excel_file(self):
        return Path(
            str(os.path.join(self.common_keywords.get_xray_utility_logs_dir_path(), ROBOFIT_TC_TRANSITION_STATE)))
    ...
    def update_custom_field_of_test_case(self, issue_ids, test_group, test_sub_group, test_type,
        ...
        try:
            ...
            for issue_id in issue_ids:
                ...
                existing_components = self.__update_components(issue_data=issue_data)
                ...
                response = self.__send_request(url=url, method=JiraApiHandler.__req_method_put,
    ...
    def update_custom_field_of_all_test_cases(self, issue_ids, test_group, test_sub_group, test_type,
                                              project_key, build_version, issue_type=ISSUE_TYPE_TEST):
        """
        Used to update the custom file of a test case.
        :return: True if successfully updated otherwise False
        :rtype: Bool
        ...
        try:
            ...
            for issue_id in issue_ids:
                ...
                if issue_type == JiraApiHandler.ISSUE_TYPE_TEST:
                    # self.set_transitions(issue_id=issue_id)
                    previous_state = self.get_current_status_of_issue(issue_id)
                    # robot_print_debug(f"{previous_state=}")
                    self.handle_test_case_transitions(issue_id, previous_state)
                    current_state = self.get_current_status_of_issue(issue_id)
                    # robot_print_debug(f"{current_state=}")
    ...
    def __update_labels(self, issue_data, updated_label_value):
        try:
            if "labels" in issue_data["fields"]:
                ...
                existing_labels_lower = [label.lower() for label in labels]

                if updated_label_value.lower() not in existing_labels_lower:
    ...
    def __update_components(self, issue_data):
        try:
            if "components" in issue_data["fields"] and issue_data["fields"]["components"] is not None:
                components = issue_data["fields"]["components"]
                existing_components = [component['name'] for component in components]
                robot_print_info(f"Existing components for issue: {existing_components}")
                return existing_components
            else:
                robot_print_info(f"No components found for issue: {issue_data['key']}")
                return []
        except KeyError as err:
            robot_print_error(f"Error to update the components, EXCEPTION: {err}")
    ...
    def __is_valid_sw_version(self, build_version):
        try:
            if build_version is not None and build_version != "":
                ...
                robot_print_error("Build version value can't be None or empty")
                return False
        except Exception as exp:
    ...
    def set_transition_on_jira(self, trans_id: str, issue_id) -> bool:
        ...
        try:
            ...
            response = self.__send_request(url=url, method=JiraApiHandler.__req_method_post,
                                           headers=headers, payload=payload)
            if response.status_code == 204:
                # robot_print_debug(f"Set the transition: {trans_id} for issue: {issue_id} is done")
                return True
            elif response.status_code == 400:
    ...
    def get_issue_creation_data(self, required_fields, test_case_type: str, test_group: str, test_sub_group: str,
        ...
        try:
            fields = {'fields': {}}
            test_case_type_key = self.get_test_issue_field_id_by_name("Test Case Type",
                                                                      self.project_config_manager.get_project_test_execution_key(),
                                                                      issue_type)
            test_group_key = self.get_test_issue_field_id_by_name("Test Group",
                                                                  self.project_config_manager.get_project_test_execution_key(),
                                                                  issue_type)
            fields['fields']['assignee'] = {"name": self.project_config_manager.jira_user_email_id()}
            fields['fields']['components'] = [{"name": self.project_config_manager.jira_get_component_name()}]
            te_label = test_case_type.capitalize().replace(" ", "_")
            fields['fields']['labels'] = [f"{te_label}_TE"]
            fields['fields'][test_case_type_key] = [{"value": test_case_type}]
            fields['fields'][test_group_key] = {
                "child": {"value": test_sub_group},
                "value": test_group
            }
            for field_key, field_name in required_fields:
                if field_name == "Issue Type":
                    fields['fields'][field_key] = {"name": issue_type}
                elif field_name == "Project":
                    fields['fields'][field_key] = {
                        "key": self.project_config_manager.get_project_test_execution_key()
                        if issue_type == JiraApiHandler.ISSUE_TYPE_TEST_EXECUTION
                        else self.project_config_manager.get_project_test_case_key()
                    }
                elif field_name == "Summary":
                    fields["fields"][field_key] = summary

            if len(fields['fields']) < 3:
                raise ValueError(f"Insufficient fields for Jira issue creation. {fields = }")

            return fields

        except Exception as exp:
            robot_print_error(f"Error generating issue creation data: {exp}")
            return {}
    ...
    def handle_test_case_transitions(self, issue_id, previous_state):
        """
        Handles transitions of a test case based on its previous state.
        Transitions:
        - If previous_state is 'Automation Debug', moves the test case to 'Released'.
        - If previous_state is 'Design Review' or 'Implementation', moves the test case to 'Implementation Review'.
        - If previous_state is 'Released' or 'Implementation Review', prints an info message indicating the test case
          is already in that state.
        - For any other state, prints an info message indicating the current state and that transitions to 'Implementation
          Review' or 'Released' are not possible.
        param issue_id: The ID of the test case issue in Jira.
        type issue_id: str
        param previous_state: The previous state of the test case.
        type previous_state: str
        """
        if previous_state == JiraApiHandler.__test_case_status_automation_debug:
            self.__move_to_released(issue_id=issue_id)
        elif previous_state in [
            JiraApiHandler.__test_case_status_design_review,
            JiraApiHandler.__test_case_status_implementation,
        ]:
            self.__move_to_implementation_review(issue_id, start_state=previous_state)
        elif previous_state == JiraApiHandler.__test_case_status_released:
            robot_print_info(f"Test case is already in {previous_state} state")
        elif previous_state == JiraApiHandler.__test_case_status_implementation_review:
            robot_print_info(f"Test case is already in {previous_state} state")
        else:
            robot_print_info(f"Current state is: {previous_state} so cannot move it to 'Implementation Review' or "
                             f"'Released'. ")

    def __move_to_implementation_review(self, issue_id, start_state):
        """
        Move the issue test case from 'New' to 'Implementation Review'.

        :param issue_id: Issue ID or key of the test case
        """
        try:
            current_state = self.get_current_status_of_issue(issue_id)
            if current_state == JiraApiHandler.__test_case_status_implementation_review:
                self.logger.debug(f"Issue: '{issue_id}' state already in 'Implementation Review'")
                return

            transitions = self.__get_transitions_to_implementation_review(start_state)

            for transition in transitions:
                self.set_transitions(issue_id, transition)
                current_state = self.get_current_status_of_issue(issue_id)

                if current_state == JiraApiHandler.__test_case_status_implementation_review:
                    self.logger.debug("Successfully moved the state to Implementation Review")

            final_state = self.get_current_status_of_issue(issue_id)
            if final_state != JiraApiHandler.__test_case_status_implementation_review:
                robot_print_error(f"Failed to move issue '{issue_id}' to 'Implementation Review', "
                                  f"current state: '{final_state}'")
        except Exception as exp:
            robot_print_error(f"Error moving issue '{issue_id}' to 'Design Review': {exp}")

    def __move_to_released(self, issue_id):
        """
        Move the issue test case from 'Automation Debug' to 'Released'.

        :param issue_id: Issue ID or key of the test case
        """
        try:
            current_state = self.get_current_status_of_issue(issue_id)
            if current_state == JiraApiHandler.__test_case_status_released:
                self.logger.debug(f"Issue: '{issue_id}' state already in 'Released'")
            elif current_state == JiraApiHandler.__test_case_status_automation_debug:
                self.set_transitions(issue_id, JiraApiHandler.TRANS_REQUIRED_STATUS_RELEASED)
            else:
                robot_print_error(f"Current state is: {current_state} which is not 'Automation Confirmed'")
        except Exception as exp:
            robot_print_error(f"Error moving issue '{issue_id}' to 'Released': {exp}")

    @staticmethod
    def __get_transitions_to_implementation_review(start_state):
        """
        Get the list of transitions needed to move to 'Implementation Review' from the given start state.

        :param start_state: The state from which to start the transition process
        :return: List of transitions needed to reach 'Implementation Review'
        """
        state_transitions = {
            JiraApiHandler.__test_case_status_design_review: [
                JiraApiHandler.TRANS_IMPLEMENTATION,
                JiraApiHandler.__test_case_status_implementation_review,
            ],
            JiraApiHandler.__test_case_status_implementation: [JiraApiHandler.__test_case_status_implementation_review],
        }
        return state_transitions.get(start_state, [])

    @property
    def test_data_dict(self):
        return self.__test_data_dict

    @test_data_dict.setter
    def test_data_dict(self, value: Tuple[str, IssueTypeTestData]):
        self.__test_data_dict[value[0]] = value[1]

    def fetch_test_data_from_jira(self, issue_keys: List[str], project_key: str, batch_size: int = 50) -> Dict[
        str, IssueTypeTestData]:
        """
        Fetches test data from Jira API for a list of issue keys in batches.

        :param issue_keys: List of Jira issue keys to fetch data for.
        :type issue_keys: List[str]
        :param project_key: Key of the Jira project to search within.
        :type project_key: str
        :param batch_size: Size of each batch of issue keys to send in a single request (default is 50).
        :type batch_size: int, optional
        :return: Dictionary mapping issue keys to IssueTypeTestData objects containing fetched data.
        :rtype: Dict[str, IssueTypeTestData]
        """
        base_url = self.project_config_manager.jira_server_domain_url().replace('"', "")
        ...
        for i in range(0, len(issue_keys), batch_size):
            ...
            query = {
                'jql': f"project = {project_key} AND issuetype=Test AND key in ({','.join(batch)})",
                'maxResult': 10000
            }
            response = self.__send_request(url=url, method=JiraApiHandler.__req_method_get,
                                           headers=headers, params=query)
            if response.status_code == 200:
                ...
                for issue in result['issues']:
                    self.test_data_dict = (issue['key'], IssueTypeTestData(
                        key=issue['key'],
                        id=issue['id'],
                        summary=issue['fields']['summary'],
                        test_type=None))
            else:
                robot_print_error(f"Failed to fetch data for batch {batch}: {response.status_code} {response.text}")

        return self.test_data_dict

    def create_issue_link(self, inward_issue_key, outward_issue_key):
        """
        Creates a link between two JIRA issues.
        Args:
            inward_issue_key (str): The key of the inward issue.
            outward_issue_key (str): The key of the outward issue.
        Returns dict or None: JSON response from the JIRA API if the link is created successfully, None otherwise.
        """
        base_url = self.project_config_manager.jira_server_domain_url().replace('"', "")
                    ...
                    "content": [
                        {
                            "content": [
                                {
                                    "text": "This issue is automatically linked while uploading the test cases and "
                                            "execution record",
                                    "type": "text"
                                }
                            ],
        ...
        try:
            response = self.__send_request(url=url, method=JiraApiHandler.__req_method_post,
                                           headers=headers, payload=payload)
            if response.status_code == 201:
                robot_print_debug(f"Issue link created successfully between {inward_issue_key} and {outward_issue_key}")
                try:
                    return response.json()
                except json.JSONDecodeError:
                    robot_print_error("Error decoding JSON response")
                    return None
            else:
                robot_print_error(f"Failed to create issue link: {response.status_code}")
                return None
        except requests.exceptions.RequestException as e:
            robot_print_error(f"Request failed: {e}")
            return None

    def check_and_create_issue_link(self, issue_key, outward_issue_key):
        """
        Checks if a link between two JIRA issues exists and creates one if it doesn't.
        Args:
            issue_key (str): The key of the issue to check for existing links.
            outward_issue_key (str): The key of the outward issue to link.
        Returns None
        """
        try:
            issue_data = self.get_issue_by_id_key(issue_key)
            if issue_data:
                issue_links = issue_data['fields'].get('issuelinks', [])
                for link in issue_links:
                    if 'outwardIssue' in link and link['outwardIssue']['key'] == outward_issue_key:
                        robot_print_debug(f"Issue link already exists {outward_issue_key}")
                        return
                self.create_issue_link(issue_key, outward_issue_key)
        except Exception as e:
            robot_print_error(f"Failed to retrieve data for the issue keys: {e}")
            return
