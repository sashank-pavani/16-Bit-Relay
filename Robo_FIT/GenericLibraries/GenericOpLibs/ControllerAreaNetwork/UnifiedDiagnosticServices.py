...
class UnifiedDiagnosticServices:
    ...
    def __create_can_bus_instance(self) -> can.interfaces.vector.VectorBus:
        ...
        try:
            ...
            return VectorBus(channel=0, app_name="CANalyzer", bitrate=500000,
                             data_bitrate=2000000, fd=True, receive_own_messages=True, sjw_abr=8, tseg1_abr=31,
                             tseg2_abr=8, sam_abr=80, sjw_dbr=6, tseg1_dbr=13, tseg2_dbr=6, output_mode=1)
    ...
    def __create_response(self, resp: udsoncan.Response) -> dict:
        """
        This Function Create Response of Dictionary and Return Response Dictionary in Format
        Example: response = {"service_name": "", "responseCode": 0, "codeName": "", "data": []}
        :return: (dict) built response dictionary
        """
        try:
            robot_print_debug(f"Res => {resp}")
            response = {"service_name": "", "service_code": "", "responseCode": 0, "codeName": "",
                        "original_payload": []}
            if resp is not None:
                try:
                    response["service_name"] = resp.service.get_name()
                    ...
                    response["service_name"] = ""
                try:
                    response["service_code"] = hex(resp.service.request_id())
                except:
                    response["service_code"] = ""
                try:
                    response["responseCode"] = hex(int(resp.code))
                except:
                    response["responseCode"] = None
                try:
                    response["codeName"] = resp.code_name
                    ...
                    response["codeName"] = ""
                try:
                    response["original_payload"] = [f"{byte:02X}" for byte in resp.service_data.value]
                except:
                    response["original_payload"] = [f"{byte:02X}" for byte in resp.original_payload]
            robot_print_debug(f"Response is : {response}")
            return response
        except Exception as exp:
            robot_print_error(f"Error to crate response Dict, EXCEPTION:{exp}")
    ...
    def read_dtc_info(self, sub_function: int, status_mask: int):
        ...
        try:
            ...
            robot_print_error(f'Server refused our request for service {e.response.service.get_name()} with code '
                              f'"{e.response.code_name}" {hex(e.response.code)}')
            return self.__create_response(e.response)
