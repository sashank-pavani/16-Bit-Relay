...
class FirstCanSignal:
    ...
    def measure_first_can_message(self):
        ...
        # log_filename = f"can_trace_{timestamp}.log"
        # log_filename = f"C:\\can_trace\\can_trace_FCM_{timestamp}.asc"
        ...
        # Stop event to control execution time


        can_bus = self.setup_can_interface(channel=0, bitrate=500000, fd_bitrate=2000000)

        if can_bus:
            # Start periodic CAN FD message sending (10ms interval)
            self.send_periodic_can_fd_message(can_bus, arbitration_id=0x597, data=[0xC0, 0x00, 0x00, 0x00, 0x21, 0x0F, 0x00, 0x00], interval=0.001)
